...
void GhostSpawner::keyPressed(int key){
    if(key == 'g'){
    std::vector<string> GhostSpawn;
    GhostSpawn.push_back("red");
    GhostSpawn.push_back("pink");
    GhostSpawn.push_back("cyan");
    GhostSpawn.push_back("orange");

    spawnGhost(GhostSpawn[ofRandom(4)]);
    }
}