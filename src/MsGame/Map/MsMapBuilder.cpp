...
MsMapBuilder::MsMapBuilder(){
	...
	pacmanSpriteSheet.load("images/MsBackground.png");
...
MsMap* MsMapBuilder::createMap(ofImage mapImage){
	...
	MsMap* mapInCreation =  new MsMap(entityManager);
    for (int i = 0; i < mapImage.getWidth(); i++) {
        for (int j = 0; j < mapImage.getHeight(); j++) {
            ...
            if(currentPixel == boundBoundBlock){
                ...
                MsPlayer* PacMan = new MsPlayer(xPos,yPos,pixelMultiplier,pixelMultiplier, entityManager);
...
ofImage MsMapBuilder::getSprite(ofImage mapImage, int i, int j){
